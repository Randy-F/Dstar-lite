   ...
   /* if (iter->second.cost == 1) glColor3f(0,1,0);
    ...
    drawCell(iter->first,0.45);*/
      if( iter->second.cost != 1 )      {
          if (iter->second.cost < 0 ) glColor3f( brightness, brightness, brightness);
          else  glColor3f(0,0,1);
          drawCell(iter->first,0.45);
       }
  }
...
void Dstar::drawCell(float _x, float _y, float size) {
...
void  Dstar::setMap()
{
    for ( int i=0; i<=99; i++ )
            for ( int j=0; j<=99; j++ )
                ...
                //    updateCell(i+30, j+40, -1);
                    map[i+30][j+40][0]=1;
}

void Dstar::robotMove()
{
   list<state>::iterator iter3= path.begin();
   iter3++;
   updateStart(iter3->x, iter3->y);
 }

void Dstar::robotUpdateMap()
{
    list<state>::iterator iter4= path.begin();

    for ( int i=0; i<=199; i++ )
        for ( int j=0; j<=199; j++ )
            if ( map[i][j][0] == 1 &&  map[i][j][1] == 0  && abs(iter4->x - i) <=4 && abs(iter4->y - j) <=4)
            {
                updateCell(i, j, -1);
                map[i][j][1] = 1 ;
            }
